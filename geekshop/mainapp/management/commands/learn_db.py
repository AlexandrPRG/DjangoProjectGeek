...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        #     .filter(Q(category__name="Диваны") |
        #             Q(category__name="Лампы"))
        # print(len(test_products))
        # db_profile_by_type('learn db', '', connection.queries)
        ACTION_1 = 1
        ACTION_2 = 2
        ACTION_EXPIRED = 3
        action_1__discount = .3
        action_2__discount = .15
        action_expired__discount = .05
        ...
        action_1__price = When(action_1__condition, then=F('product__price') * F('quantity') * F(action_1__discount))
        action_2__price = When(action_2__condition, then=F('product__price') * F('quantity') * F(action_2__discount))
        action_expired__price = When(action_expired__condition, then=F('product__price') * F('quantity') * F(action_expired__discount))
